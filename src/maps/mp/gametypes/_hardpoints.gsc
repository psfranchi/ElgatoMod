...
//Default the granade launcher stock ammo to 0, so only will have 1 on the tube
removeExtraGlAmmo(){
	allweapons = self GetWeaponsList();
	for( i = 0; i < allweapons.size; i++) {
		weapon = allweapons[i];
	  if(isSubStr( weapon, "gl_" ) && !isSubStr( weapon, "_gl_" ))
		{
...
avoid3Nades()
{
	sg_concussion_stock = self GetWeaponAmmoStock("concussion_grenade_mp");
	sg_flash_stock = self GetWeaponAmmoStock("flash_grenade_mp");
...
onClassChange()
{
	...
	for(;;)
	{
		...
		{
			if ( isSubStr(response, "custom1") ||
				 isSubStr(response, "custom2") ||
				 isSubStr(response, "custom3") ||
				 isSubStr(response, "custom4") ||
				 isSubStr(response, "custom5") ||
				 isSubStr(response, "assault_mp") ||
				 isSubStr(response, "specops_mp") ||
				 isSubStr(response, "heavygunner_mp") ||
				 isSubStr(response, "demolitions_mp") ||
				 isSubStr(response, "sniper_mp")
			)
			{
				self waittill( "weapon_change", newWeapon );
				self avoid3Nades();
